...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM categories WHERE name = 'Lighting Control') THEN
    INSERT INTO categories (name, description, color)
    VALUES ('Lighting Control', 'Lighting control systems and interfaces', 'purple');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM categories WHERE name = 'Audio Control') THEN
    INSERT INTO categories (name, description, color)
    VALUES ('Audio Control', 'Audio mixing and control equipment', 'blue');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM categories WHERE name = 'Display Systems') THEN
    INSERT INTO categories (name, description, color)
    VALUES ('Display Systems', 'Screens, projectors, and displays', 'green');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM categories WHERE name = 'Communication') THEN
    INSERT INTO categories (name, description, color)
    VALUES ('Communication', 'Communication and intercom systems', 'yellow');
  END IF;
END $$;

-- Add unique constraint on items.name if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
